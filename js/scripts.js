...
// emailjs
emailjs.init("7LEW-gXEQ5ExKx9_9"); // Substitua pela sua Public Key
...
form.addEventListener("submit", function (event) {
  ...
  emailjs.send(serviceID, templateID, formData)
    .then(() => {
      form.reset();
      mensagem.style.display = "block";
      oculosAnimado.style.animation = "none"; // reset animation
      void oculosAnimado.offsetWidth; // trigger reflow
      oculosAnimado.style.animation = "flutuar 1.5s ease-out forwards";
    })
    .catch((error) => {
      console.error("Erro no envio:", error);
